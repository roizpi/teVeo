var Weather = (function(_super,$,environment){
	...
	function Weather(geoLocation){
		...
		//Obtenemos referencia al localizador de servicios.
	...
	/*
		...
		$api_key = '6d2994aa361e6b53e25ea940efa73ee8';
	...
	*/
	...
	var requestData = function () {
		...
		self.geoLocation.getCoords().pipe(function(coords){
	...
	var ForecastConditions = function(raw_data) {
		...
		//Devuelve el icono de la condición.
	...
	*/
	Weather.prototype.getCurrentConditions = function () {
		...
		if(data !== false) {
			return new ForecastConditions(data.currently);
		} else {
			return false;
		}
	};
	...
	/**
	* Devuelve las condiciones para cada hora
	*
	*/
	Weather.prototype.getForecastToday = function () {
		data = requestData();
		if(data !== false) {
			conditions = [];
			...
			for(i=0; i<data.hourly.data.length; i++) {
				...
				if(moment.unix(raw_data.time).format("YYYY-MM-DD") == today) {
					conditions.push(new ForecastConditions(raw_data));
				}
			}
			return conditions;
		} else {
	...
	/**
	*
	*	Retorna las condiciones para los próximos 7 días.
	*/

	Weather.prototype.getForecastWeek = function () {
		data = requestData();
		if(data !== false) {
			conditions = [];
