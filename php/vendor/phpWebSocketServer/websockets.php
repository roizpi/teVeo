...
abstract class WebSocketServer {
  ...
  function __construct($addr, $port, $bufferLength = 5000) {
    ...
    socket_listen($this->master,null)                               or die("Failed: socket_listen()");
      ...
      foreach ($read as $socket) {
        ...
        if ($socket == $this->master) {
          ...
          $numBytes = socket_recv($socket,$buffer,$this->maxBufferSize,0);
            echo "Hola estoy aquíiíííí" .PHP_EOL;
          if ($numBytes === false) {
  ...
  protected function disconnect($socket, $triggerClosed = true) {
    ...
    if ($disconnectedUser !== null) {
        $this->users->detach($disconnectedUser);
